...
def remove_prima_volta(score, change_measure_number=True):
    """Works inplace due to the call to an inplace function in music21"""
    for p in score.parts:
        # TODO: This works only for prima and seconda volta, not for more complex cases
        repeat_brackets = [rb for rb in p.getElementsByClass(music21.spanner.RepeatBracket)]
        to_remove = []
        for rb in repeat_brackets:
            if rb.getNumberList() == [1]:
                measures = rb.getSpannedElementsByClass(music21.stream.Measure)
                if len(measures) > 2:
                    for m in measures:
                        to_remove.append(m)
                elif len(measures) == 2:
                    ...
                    start_end = rb.getSpannedElementsByClass(music21.stream.Measure)[0]
        ...
        if change_measure_number:
            total_measures = p.getElementsByClass(music21.stream.Measure)
...
class AnnotationConverter(ABC):
    def __init__(self, in_ext=None, out_ext=None):
        ...
        self.quality_music21_to_tab = {
            ...
            "augmented sixth": "+6",  # This should never happen!!
            "German augmented sixth chord": "Gr+6",
            "French augmented sixth chord": "Fr+6",
            "Italian augmented sixth chord": "It+6",
            "quartal trichord": "sus",  # TODO: Check if this is correct!
            # 'Neapolitan chord': 'N6'  # N/A: major triad  TODO: Add support to Neapolitan chords?
            # Following are hacks, i.e., mapping that are not entirely correct but simplify the task
            "dominant-ninth": "D7",  # Simplification
            "augmented major tetrachord": "a",  # Simplification
            "minor-augmented tetrachord": "m",  # Simplification
            "major-second minor tetrachord": "m",  # Simplification
        }
    ...
    def chord_music21_to_tab(self, rn):
        def _get_full_degree(rn):
            ...
            def _degree_str_from_rn(rn):
                ...
                # FIXME: music21 doesn't always assign augmented sixths to degree +4. Sometimes
                #  it forgets the sharp, sometimes it just uses different degrees.
                #  We hack the correct degree +4 in.
        ...
        def _get_quality(rn):
            ...
            # TODO compress
            # music21 uses brackets to specify some extensions or omissions, discard those
